...
class Task:
    ...
    def finish(self, data):
        ...
        if not self.finish_time:
            print "finish(): finish_time: ", self.finish_time
            self.finish_time = 0
    ...
    def report(self, indent):
        ...
        if self.finish_time and self.launch_time:
            s += pfx + "Run time: {}ms\n".format(self.finish_time - self.launch_time)
            self.runtime = self.finish_time - self.launch_time
        #TODO remove this
        if self.has_metrics:
            self.runtime = self.executor_run_time
